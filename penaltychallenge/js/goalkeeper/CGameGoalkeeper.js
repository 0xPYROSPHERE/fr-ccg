function CGameGoalkeeper(iLevel,iCurBallForceY,oParentContainer) {
    ...
    this._init = function (iLevel,iCurBallForceY) {
        ...
        _bBallOut = false;
        _bPostHit = false;
        _bBallStoppedAfterLaunch = false;
    ...
    this.activeEventListeners = function () {
        if (SHOW_3D_RENDER) {
            ...
            _oListenerMouseDownStage = _oContainerGame.on("mousedown", this.onMouseDown);
    ...
    this.onMouseDown = function(){
        if (_oStageMouseMove === null) {
            _oStageMouseMove = s_oStage.on("stagemousemove", s_oGameKeeper.onHandKeeper);
        }
        _oContainerGame.off("mousedown",_oListenerMouseDownStage);
    };
    ...
    this._goalMiss = function(){
        ////WHEN BALL STOP PROBABLY WITH A POST HIT
        _bBallStoppedAfterLaunch = true;
        _bPostHit = true;
    };

    this._ballOut = function(){
        _bBallOut = true;
    };
    ...
    this._checkBallState = function(){
        ...
        if(oBallBody.velocity.lengthSquared() < 0.01){
            if(_bLaunched && !_bBallStoppedAfterLaunch && !_bBallOut && !_bGoal){
                this._goalMiss();
            }
            return;
        }
    ...
    this._updatePlay = function () {
        ...
        if (_bKeeperSave || _bGoal || _bBallOut || _bPostHit) {
            this.timeReset();
        }
